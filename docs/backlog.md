    ...
    - **Phasing (added 2026-08-20):**
      ...
      - **56b — Metering + limits:** per-account token metering, caps with the fair-use policy, rate limiting, abuse controls, margin dashboard for the maintainer; the app shows remaining allowance and the "switch to your own key for unlimited" valve.
        ...
        - **Build slices (scoped 2026-08-29):**
          ...
          4. **Docs/tests:** `docs/managed-inference.md` "Metering" section (window semantics, config vars, error codes, privacy statement); Worker unit tests (window rollover, rate limiter, safety cap, soft vs hard) with vitest; app tests for quota decoding, error mapping, threshold-alert idempotence. — ✅ **app half done** *(branch `usage-metering`)*: app-side "Metering (56b)" doc section + app tests (quota decoding present/absent, error mapping incl. `Retry-After`, threshold-alert idempotence + window reset, progress-text formatting); ✅ **Worker half done** *(`sentwise-service` branch `usage-metering`: 73 vitest tests — window rollover, rate limiter, safety cap, soft/hard, admin endpoint; README "Metering (56b)" section)*. **Remaining to close 56b:** merge both branches, deploy the Worker, set the two secrets, and live-verify a managed draft returns `quota` + the pane renders it.
