...
83. **Approval-signal learning loop (accept-as-is / edit / deny → better drafts + smarter filtering)** — *ongoing/strategic; phase 1 is a cheap early slice*
    ...
    *As Priya/Marcus, I want Sentwise to learn from how I handle its drafts — what I accept as-is, what I rewrite, what I throw away — so that over time it drafts more of what I'd actually send and stops surfacing mail I never reply to.*
    ...
    - **Phase 1 — capture the signal (cheap, non-destructive, worth doing sooner):** distinguish approved-as-is vs approved-after-edit (fold `wasEdited` + edit magnitude into the approval event / a small on-device feedback store); record deny with a lightweight optional reason (quick-pick: "not worth replying" / "wrong tone" / "wrong content" / "handle later", or silent); attribute each signal to draft provenance (watcher vs "Draft anyway" override — needs item 68).
    ...
    - **Privacy (load-bearing):** all signal capture, storage, and learning stay **on-device** — this is the user's own data improving their own experience locally, fully consistent with no-storage / no-training; it is never a shared or trained model, and any inference still rides the normal per-draft call. Ties to items 17/18/19/20/21/34/58/66/67/68.
