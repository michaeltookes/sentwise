...
72. **Legal, policy, and support foundations for a paid launch** — *⚠️ partly blocked (2026-08-28): don't start wholesale yet*
...
83. **Approval-signal learning loop (accept-as-is / edit / deny → better drafts + smarter filtering)** — *ongoing/strategic; phase 1 is a cheap early slice*
    ...
    *As Priya/Marcus, I want Sentwise to learn from how I handle its drafts — what I accept as-is, what I rewrite, what I throw away — so that over time it drafts more of what I'd actually send and stops surfacing mail I never reply to.*
    ...
    - **Phase 1 — capture the signal (cheap, non-destructive, worth doing sooner):** distinguish approved-as-is vs approved-after-edit (fold `wasEdited` + edit magnitude into the approval event / a small on-device feedback store); attribute each signal to draft provenance (watcher vs "Draft anyway" override — needs item 68).
      - **Deny-reason capture (owner decision 2026-08-28):** hitting **Deny** presents a **reason picker** (single-select) before the deny finalizes: preset choices — "Not worth replying" / "Wrong tone" / "Wrong content" / "Handle later" (final list TBD) — plus an **"Other"** option. Selecting **"Other" reveals a MANDATORY free-text field**; the deny cannot complete until a reason is chosen and, for Other, non-empty text is entered. Reason (and any free text) is stored with the deny signal.
      - **Friction guard:** heavy deniers shouldn't face a modal on every deny — consider a "remember my last reason" or a per-session "don't ask again" affordance so the picker stays helpful, not annoying. The picker is a state-mutating flow → Prowl-forbidden like the existing Deny action; the reason controls get AX identifiers.
