...
83. **Approval-signal learning loop (accept-as-is / edit / deny → better drafts + smarter filtering)** — *ongoing/strategic; phase 1 is a cheap early slice*
    ...
    *As Priya/Marcus, I want Sentwise to learn from how I handle its drafts — what I accept as-is, what I rewrite, what I throw away — so that over time it drafts more of what I'd actually send and stops surfacing mail I never reply to.*
    ...
    - **Privacy (load-bearing) — two distinct layers, do not conflate:**
      ...
      - **Cross-user aggregation (to "revise the draft algorithm as we gain users") is a SEPARATE, opt-in path.** Sending deny reasons — especially the "Other" free text, which can contain mail content — off-device to inform product-wide drafting is **telemetry** and must go through item 35's opt-in, disclosed, content-scrubbed channel. It must never happen silently; doing so would break the no-storage/no-training promise. Structured reason *codes* (no free text) are far safer to aggregate than raw text; the free-text field is primarily for local review and per-user learning.
