...
78. **Surface disabled notification permission (primary approval channel fails silently)**
    When macOS notification authorization is off/denied, the app posts draft notifications that never appear, silently breaking the core approve-from-notification workflow with no indication to the user.
    > **Discovered 2026-08-24/25** during the item 61 live loop observation: the watcher drafted replies correctly, but no banner ever appeared. Root cause: **"Allow Notifications" was OFF** for Sentwise in System Settings, so `NotificationService.notify(...)` posted requests macOS silently dropped. The app requests authorization once at launch (`AppDelegate` → `NotificationService.requestAuthorization`, options `[.alert, .sound]`) and only logs the granted/denied result — it never re-checks the live authorization status or tells the user when it's denied. On the shipped signed build the first-launch prompt fires, but any user who dismisses/denies it (or later turns it off) gets a silently non-functional primary approval channel.
    *As Priya, I want the app to tell me when notifications are off and how to turn them on, so that I actually receive the draft approvals the product is built around instead of wondering why nothing happens.*
    - On launch (and when returning to foreground / opening the menu), check `UNUserNotificationCenter` authorization status; when it is `.denied` or `.notDetermined`, surface a clear, non-nagging indicator (e.g. a banner in the menu/Review Drafts window and/or a Settings row): "Notifications are off — you won't see draft approvals" with a button that deep-links to System Settings → Notifications → Sentwise (`x-apple.systempreferences:com.apple.Notifications-Settings.extension`).
    - Re-request authorization where the status is `.notDetermined`; for `.denied`, guide to System Settings (the OS won't re-prompt).
    - Do not spam: show the hint only while notifications are off; clear it once granted.
    - Consider a lightweight fallback so a denied user isn't fully stuck (e.g. the Review Drafts window already works as the approval surface — make sure that path is discoverable when notifications are off).
    - Prowl/hunt mode unaffected (hunt mode already stubs notifications).
    - Verify on a real signed build that the first-launch prompt appears and that toggling the setting off later surfaces the indicator.

79. **Rework the draft notification into an "Open / Close" alert (approve in the app, not from the banner)**
    The notification banner can't show the full drafted reply, so approving directly from it is a blind approve. Change the notification to a simple alert whose only actions are **Open** (open Review Drafts to read + approve the full draft) and **Close** (dismiss); move all approval into the app.
    > **Owner decision 2026-08-25** (during the item 61 live observation): the banner truncates the draft, so one-tap approve/deny from the notification is unsafe. This **supersedes the one-tap-approve-from-notification model** for the notification surface specifically, and supersedes the earlier (unbuilt) "relabel the notification's approve action to 'Approve'" idea — the notification will have **no** approve/deny action at all. The Review Drafts **window keeps its "Approve" button** (and the earlier decision to label the in-app approve action "Approve" everywhere in-app still stands and should be folded into this work).
    *As Priya, I want the notification to just tell me a draft is ready and let me open it, so that I read the full reply before approving instead of approving something I can't see.*
    - The draft notification becomes a general alert: title/sender/subject as now, actions reduced to **Open** and **Close** only. Remove the Approve/Deny (and send-behavior-specific "Send"/"Save to Drafts") actions from the notification.
    - **Open** activates the app and opens the Review Drafts window focused on that draft (reuse the existing custom-URL / open-review path); **Close** dismisses the notification with no side effects.
    - Approval, deny, regenerate, and send-behavior handling all happen in the Review Drafts window, which already shows the full draft; ensure that surface is complete and discoverable as the sole approval path.
    - **In-app approve button reads "Approve"** (folds in the earlier decision) on the Review Drafts card and the manual draft-preview sheet; the window's "Approve will save to drafts / will send" caption still conveys the send behavior.
    - Auto-send mode: since approval now happens in-app, the auto-send + undo-countdown flow runs from the window, not the notification; verify the undo affordance still works on that path.
    - **Docs to update when built:** CLAUDE.md and this backlog describe *"one-tap approval via a native macOS notification"* as the core promise — update that language to reflect notify → open → approve-in-app (the notification alerts; approval is a deliberate in-app action).
    - Update `NotificationService` (`approveActionTitle`, category/action registration, `notify`/`refreshNotification`) and its tests; keep Prowl hunt mode's notification stub working.
