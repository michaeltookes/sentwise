...
93. **Brand parity: app icon + DMG art on the "Closer" palette** — *owner-raised 2026-09-12; land before item 74 cuts 1.0*
    ...
    *As a first-time user coming from sentwise.ai, I want the app I install to look like the product I was sold, so that nothing feels off between the site and the app.*
    ...
    - ✅ **Built — pending owner visual review** *(branch `closer-brand-assets`, 2026-09-12)*: recolored the app icon + DMG art to the Closer palette from the site's `@theme` tokens (forest `#0A3D2D`/`#072A20`, emerald `#3FD99B`/`#0B6E47`, cloud `#EAF4EE`, gold `#F2B705`). `Distribution/assets/Sentwise.svg` rebuilt as the site's dark-ground owl on the macOS icon grid (emerald-outline envelope, cloud eyes, gold beak); `Sentwise-small.svg` reduced to the 16/32px cues that survive downscale (filled emerald envelope, solid forest eyes, enlarged gold beak). New `Distribution/scripts/make-appiconset.sh` single-sources all 10 appiconset slots from the SVG masters; all slots + `icon_1024.png` regenerated and the icon verified baked into a local build. `dmg-background.svg` recolored to the Closer *light* treatment (paper ground, emerald weave/arrow, ink title) so the dark icon tile and Applications folder stay legible on both Finder themes; PNGs re-exported at the exact 660×400 / 1320×800 with baked geometry untouched. Menu-bar status item is the `envelope.badge` SF Symbol with `isTemplate = true` (monochrome) — no palette change needed. **Not yet resolved:** awaiting owner sign-off on the rendered icon (Dock/notification/Finder) and a locally built DMG.
