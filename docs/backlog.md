...
97. **Live-verification pipeline: env-gated live suite on the Lucius runner + `/live-verify` dispatch skill** — *owner-requested 2026-09-14; the standing post-build live-testing mechanism (all live testing delegated to Lucius — never the owner's Mac)*
    Make live verification a repeatable pipeline instead of owner click-throughs: a GitHub Actions workflow that runs the env-gated live test suite on the Lucius Mac mini self-hosted runner (headless, always-on, GUI session for the window server), plus a thin skill that dispatches it from any agent session. Division of labor: **the Action is the executor and system of record; the skill is only a dispatcher** (`gh workflow run` + poll + report) and never executes tests locally.
    *As the maintainer, I want every feature live-verified on Lucius after it's built — automatically on merge and on demand for a branch — so that my own Mac is never locked up and live regressions are caught without me clicking through flows.*
    - **Workflow** (`live-tests`): `runs-on: self-hosted` (Lucius); triggers = `workflow_dispatch` with a ref input (branch runs, invoked by the skill) + push to `main` (every merge re-verified at its triggering `github.sha`). NOT per-PR-commit — live tests hit real services and drafts cost real Anthropic spend. The single Lucius runner serializes live jobs; do not add a global Actions concurrency group because GitHub can cancel older pending runs even with `cancel-in-progress: false`.
    - **Env plumbing:** `SENTWISE_LIVE_*` as repo secrets; the workflow encodes the documented app-hosted-test recipe (`build-for-testing` → inject env into `.xctestrun` → `test-without-building`) since plain shell env does not propagate (see `docs/managed-inference.md`).
    - **Scope guardrail (standing):** the live suite covers sign-in, drafting, the checkout-overlay-loads check, and portal-link fetches. It NEVER completes a purchase, enters card data, or touches billing change controls (2026-09-10 billing-guardrail decision) — real purchases stay rare, human, deliberate.
    - **First payload:** a `SENTWISE_LIVE_PADDLE_CHECKOUT`-gated test that instantiates the real checkout WebView with the real `CheckoutNavigationPolicy` delegate against Paddle sandbox and asserts the overlay reaches Paddle's real `checkout.loaded` event — this closes item 95's pending live verification of the navigation policy, and catches the policy-too-tight regression class permanently.
    - **Fold in existing live-gated tests:** `ClerkLiveSignInTests`, and the long-parked item 44 att.net save-as-draft verify finally gets its home.
    - **`/live-verify` skill:** dispatches the workflow for a given ref, waits/polls, reports pass/fail + failure excerpts; never runs tests on the invoking machine. Wired into the kickoff lifecycle as a final step after `/validate-feature` passes and the branch is pushed (result included in the builder's report); merges are covered by the push-to-main trigger automatically.
    - **Runner hygiene:** same constraint as prowl-qa — the runner must be started from Lucius's on-console GUI session (`svc.sh` from the Aqua session, never SSH/sudo) or WebKit/window-server access fails; document alongside the existing runner notes.
    - Docs: a short `docs/live-testing.md` covering the workflow, secrets, dispatch, and scope guardrail.
    - ✅ **Built — pending first green run on Lucius** *(branch `live-verification`, 2026-09-14)*: the pipeline is built and validated offline; it is **not resolved** because it has not yet had its first green run on the Lucius runner (that needs owner-side secret provisioning + a runner started from the on-console GUI session). Shipped:
      - **First payload** `PaddleCheckoutLiveTests` (`SentwiseTests/PaddleCheckoutLiveTests.swift`, gated on `SENTWISE_LIVE_PADDLE_CHECKOUT`, skips offline): loads the real `PaddleCheckoutHTML` harness in a `WKWebView` under the real `CheckoutNavigationPolicy` rule, opens the **Paddle sandbox** overlay via the harness's own `window.sentwiseOpenCheckout` by `items` (client-token + sandbox price id — no purchase, no card, no server transaction), and asserts the overlay reaches Paddle's real `checkout.loaded` event while the policy blocked no Paddle navigation. Closes item 95's *pending live* check and catches the policy-too-tight regression class. (Opened by `items`, not a live-Clerk→Worker-minted transaction, because the overlay + sub-frames render identically either way with far fewer live dependencies; the transaction-id chain is a future payload once the Worker holds a sandbox Paddle key.)
      - **Workflow** `.github/workflows/live-tests.yml`: `runs-on: [self-hosted, macOS]` (mirrors `prowl-qa.yml`); triggers = `workflow_dispatch` (ref input) + push to `main`; no global Actions concurrency group, so pending dispatches are preserved and serialized by the Lucius runner queue. Steps: checkout dispatch input ref or push-triggering `github.sha` → `build-for-testing` → inject secrets into the `.xctestrun` → `test-without-building` scoped with `-only-testing` to the live classes → upload xcresult on failure.
      - **Env plumbing** `Distribution/scripts/inject-live-env.sh` (shellcheck-clean): writes each provisioned `SENTWISE_LIVE_*` secret into the xctestrun `EnvironmentVariables` (names-only logging; handles FormatVersion 1/2; `-o -` on all probes so it never corrupts the file). `ManagedInferenceLiveTests` now uses `SENTWISE_LIVE_MANAGED_INFERENCE` plus `SENTWISE_LIVE_CLERK_TEST` to mint a fresh Clerk session JWT during the run rather than storing a short-lived JWT as a repo secret; the `/v1/draft` spend check has the extra `SENTWISE_LIVE_MANAGED_DRAFT` gate and dedicated `SENTWISE_LIVE_MANAGED_DRAFT_EMAIL` so it only runs against a Clerk test user with durable Worker entitlement or trial bypass.
      - **Folded-in live classes** in `-only-testing`: `PaddleCheckoutLiveTests`, `ClerkLiveSignInTests`, `ManagedInferenceLiveTests`, `GmailLiveSendTests`, `ReplyWorthinessLiveTests`, `AttNetLiveDraftTests` (item 44's save-as-draft verify now has its home).
      - **`/live-verify` skill** (`.claude/skills/live-verify/SKILL.md`): dispatch-only (`gh workflow run` + watch + `--log-failed` report); states it never runs tests locally; carries the no-purchase scope guardrail; compares local and remote branch object ids before dispatch and passes the exact pushed SHA as the checkout input so Lucius cannot verify stale branch code.
      - **Docs**: `docs/live-testing.md` (pipeline runbook), plus `docs/live-verification.md` and `docs/managed-inference.md` updated.
      - **Validated offline**: full suite 1862 tests, 10 skipped (all live gates absent, incl. `PaddleCheckoutLiveTests`), 0 failures; shellcheck + actionlint clean. No live test was run on the owner's Mac.
      - **Review hardening (2026-09-15)**: added an env-gated read-only `/v1/paddle/manage-billing` portal-link payload so the documented live-scope claim is exercised, validates that it is a fresh authenticated Paddle customer-portal-session URL instead of a generic sign-in fallback, removed the contradictory local `xcodebuild` live-test recipe from the runbook, and made `ReplyWorthinessLiveTests` skip instead of failing when the sampled mailbox has known transactional mail but no worthy message. Focused validation: `git diff --check`, `bash -n`, `shellcheck`, `actionlint`, scoped `swiftlint`, and focused `xcodebuild test` (7 selected tests, 6 expected live-gate skips, 0 failures).
      - **Additional `/live-verify` review hardening (2026-09-15)**: query fully qualified remote branch/tag refs so nested branch names cannot suffix-match the requested ref, default a detached checkout to its exact commit SHA instead of the literal `HEAD`, and dispatch historical branch/tag refs from the default branch workflow when the requested remote ref lacks `.github/workflows/live-tests.yml` while still checking out the pinned requested OID. Validated with `git diff --check`, a detached-HEAD fallback dry run, and a networked remote-ref dry run against `origin/live-verification`.
      - **Further review hardening (2026-09-15)**: reject local tags that differ from the same-named remote tag before live dispatch, and wrap the att.net live draft save itself in the cleanup path so an ambiguous IMAP `APPEND` failure cannot leave a test draft behind. Validated with `git diff --check`, scoped `swiftlint`, branch + tag remote-ref dry runs, and focused `xcodebuild test` for `AttNetLiveDraftTests` (1 expected live-gate skip, 0 failures).
      - **Owner provisioning to close this item:** (1) add the repo secrets in `docs/live-testing.md`'s checklist (at minimum `SENTWISE_LIVE_PADDLE_CHECKOUT=1`, `SENTWISE_LIVE_CLERK_TEST=1`, and `SENTWISE_LIVE_MANAGED_INFERENCE=1` if exercising the Worker payload; add `SENTWISE_LIVE_MANAGED_DRAFT=1` plus `SENTWISE_LIVE_MANAGED_DRAFT_EMAIL=<entitled +clerk_test account>` only after the draft test user has durable entitlement; add `SENTWISE_LIVE_MANAGED_PORTAL=1` plus `SENTWISE_LIVE_MANAGED_PORTAL_EMAIL=<subscribed +clerk_test account>` after a subscribed Paddle sandbox account exists; mailbox creds for the Gmail/att.net payloads); (2) confirm `sentwise.ai` is an approved domain in the Paddle **sandbox** account; (3) ensure the Lucius runner is started from its on-console GUI session; (4) run `/live-verify` (or merge to `main`) and confirm the first green run, then move this item to resolved.
...
92. **Surface scheduled cancellation ("Cancels on <date>") in the Subscription pane** — *discovered 2026-09-12 during the items 90/91 cancel live-verify*
    Canceling from the Paddle portal defaults to **cancel at end of billing period**: Paddle fires `subscription.updated` with `status` still `active` plus `scheduled_change: {action: "cancel", effective_at: …}`. The worker's webhook maps only `status` and ignores `scheduled_change`, so `/v1/me` reports a plain active subscription and the pane shows nothing about the pending cancellation until the period actually ends (when `subscription.canceled` fires and the existing canceled-state handling kicks in). Observed live: owner canceled in the portal and the app kept showing the active Pro plan with no hint the cancel had registered.
    *As a paying user who cancels, I want the app to say my plan ends on \<date\> the moment I cancel, so that I trust the cancellation took effect and know how long I keep access.*
    - Worker: parse `scheduled_change` (at least `action: "cancel"` + `effective_at`) from subscription webhook events and store it in `privateMetadata.subscription`; clear it when a later event carries no scheduled change (cancellation undone in the portal). Explicitly decide/ignore `pause`/`resume` scheduled actions.
    - `/v1/me` exposes the scheduled cancellation (e.g. `subscription.cancelsAt`).
    - App: Subscription pane shows "Cancels on \<date\>" on the current plan (and suppresses upgrade prompts as appropriate); state clears if the user reverses the cancellation. Prowl hunt-safe; AX ids preserved.
