...
98. **Tier feature matrix — decide what each plan gets beyond credits**
    ...
    *As the maintainer, I want a committed feature-by-tier matrix before building any gated feature, so that pricing, the landing page, entitlement wiring, and the build order all follow one decision instead of drifting.*
    ...
    - **Decided 2026-09-16 (owner):** connected email accounts per tier — **Starter 1, Pro 2, Unlimited 5** ("Unlimited" refers to follow-up volume, not accounts). Allotments stand as marketed on the pricing page: 30 / 120 / unlimited-fair-use follow-ups per month. Pricing cards only ever show shipped capabilities — no "coming soon" (site ticket: landing-page item 14).
