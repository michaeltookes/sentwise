...
- **LLM access:** **managed inference only** (bundled, no keys; item 56). *(Superseded 2026-09-16: the BYO-any-provider and local-model paths are parked — removed from UI and pricing, code kept — see item 100.)*
- **Ethos:** local-first, private. Mail data, voice profile, and (future) call audio stay on the user's machine; drafting inference leaves only as a stateless, zero-retention call via the managed proxy (the sole shipped path since 2026-09-16 — item 100). *(The original "no subscription" and BYO-key-by-default clauses were superseded 2026-08-12 — see Monetization above.)*
...
108. **Inbox drafting policy: tier-gated watching, draft-on-click default, opt-in auto-drafting with sender rules + budget cap** — *owner decision 2026-09-21; launch-blocking (ships before 1.0), from the usage-burn discussion*
    Auto-drafting spent the paid allotment without explicit intent (a denied draft cost the same as a sent one), and Starter's "30 follow-ups a month" could be consumed in days by an ordinary inbox. Owner decisions: **Starter = transcripts + manual only** (no inbox watcher at all — no reply-worthiness gate, no notifications, zero background token burn; manual on-demand drafting from Browse/preview stays, one credit per ask). **Pro/Unlimited/Trial keep the watcher with notification-first, draft-on-click as the default** (spend = intent), and **automatic drafting becomes an explicit opt-in** with real controls. Post-call transcript follow-ups auto-draft on every tier — dropping a transcript is the intent.
    *As a Starter user, I want my 30 monthly follow-ups spent only on drafts I ask for, so that my plan lasts the month. As a Pro user, I want reply-worthy email to notify me and draft on my click — with auto-drafting I can turn on for chosen senders and cap with a budget — so that the watcher works for me without spending for me.*
    - **Tier gate (client-side, keyed off `subscription.plan` like `AccountConnectionLimit`; same accepted A-L5 posture):** watchers (focused + background) never start on Starter; Settings communicates "Inbox watching is a Pro feature." Unknown/offline plan follows the account-gate precedent (Pro-equivalent default).
    - **Draft-on-click (watcher tiers' default):** a reply-worthy message produces a notification ("Reply-worthy: <sender> — draft a reply?") and a pending *undrafted* entry in Review Drafts; generation (and the credit spend) happens only on explicit request. Design the intermediate "worthy, awaiting request" state in the review window; hunt-safe; AX ids.
    - **Automatic drafting (Pro/Unlimited, opt-in, default OFF):** Settings toggle plus per-sender/domain **auto-draft list** (grow item 18's allowlist rules: "always auto-draft these"), and an optional **monthly auto-draft budget cap** ("auto-drafting may use at most N of my allotment") with usage-alert integration. Trial gets the same controls.
    - **Reauth re-baseline:** resuming watchers after managed sign-in/reauth re-baselines instead of processing the accumulated backlog (mirror the reconnect behavior) — kills the catch-up burst observed 2026-09-21.
    - **Docs/pricing:** `docs/tier-matrix.md` rows updated (done with this item's logging); landing item 14 amended; app plan-picker copy reflects the split; `docs/managed-inference.md` spend-semantics note.
    - Tests: per-plan watcher gating, spend-on-click timing (no generation before request), auto-draft list matching, budget-cap enforcement, reauth baseline, migration of existing users' settings (existing installs on watcher tiers default to draft-on-click, NOT silently to auto).
    - Ties to items 18 (rules), 79 (notifications), 83 (approval signal — "requested vs ignored notification" becomes a new signal), 98/99 (tier matrix), 84 (usage view).
...
74. **Launch readiness: clean-Mac verification, security pass, and the 1.0 release**
    ...
    *As the maintainer, I want proof that a first-time user on a clean Mac gets from download to first draft without help, so that launch day isn't debugging day.*
    ...
    - **Production Paddle cutover** (**unblocked 2026-09-12 — Paddle vendor account fully verified**; from item 56): point `PaddleConfig.active` / `PADDLE_API_BASE` at the live token + price ids, and mint the **live API key with the `customer_portal_session.write` permission** — discovered 2026-09-12 (item 91): without it the portal-session create fails silently and billing links regress to the email sign-in page. Also: production Clerk instance needs a real Google OAuth client + its own `/auth/callback` redirect-allowlist entry (item 89), and real per-tier allotment numbers set from 56b measurements. **Tier-matrix alignment (from item 98, 2026-09-16):** live Paddle products/prices must match `docs/tier-matrix.md` exactly (Starter $9 / Pro $19 / Unlimited $39, plan ids `starter`/`pro`/`unlimited`); the quota window-unit mismatch is resolved by the Worker `window-unit-fix` (calendar-month UTC, resets 00:00 UTC on the 1st) and the item 101 app/docs alignment, so no launch cutover action remains for the window unit.
    - Launch checklist recorded in `docs/` and ticked; this item closes when the public link goes out.

104. **Bundled Google OAuth client + CASA: passwordless Gmail connect** — *logged 2026-09-19 (owner decision); post-launch build, queue-front with item 53 once 1.0 ships*
    Replace the app-password onboarding step with one-click "Sign in with Google" using a Sentwise-owned verified OAuth client — the revival of the parked item 3 engine in the configuration that avoids its BYO-credentials friction entirely.
    *As a new user, I want to connect Gmail by approving a Google consent screen, so that I never generate, copy, or paste an app password.*
    - **Why now:** the app-password flow (2FA prerequisite + 16-char paste) is the top-of-funnel friction point for the Marcus ICP, and Workspace admins can disable app passwords outright (item 75's gap). Owner assessment 2026-09-19: acceptable for the first 10–50 users, won't scale past that.
    - Revive the parked item 3 engine (`GmailAuthCoordinator`, `OAuthTokenService`, PKCE loopback flow — built and unit-tested; fix the known `NWError 22` loopback-listener start bug), swapped from BYO client config to one bundled Sentwise client.
    - IMAP/SMTP auth via **XOAUTH2** with the `https://mail.google.com/` scope; token refresh + Keychain storage already built. App password stays as a documented fallback path (and the path for non-Gmail providers).
    - **The gate is Google's restricted-scope verification + annual CASA assessment** (the `mail.google.com` scope is restricted): third-party assessment through a Google-authorized lab, recurring yearly. Scan-based Tier 2 assessments run roughly $500–$1,000/yr (e.g. TAC Security); full lab audits $4.5k–$15k+. Verify current pricing/tier assignment at submission time. Google's end-to-end verification commonly takes 2–6+ weeks — an external clock; submit as soon as the integration demos.
    - Local-first architecture is a strong CASA fit (no server touches mail; tokens in Keychain) — the assessment story is documentation, not re-architecture.
