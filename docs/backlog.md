...
77. **Watcher IMAP fetch leaks an unfulfilled `EventLoopFuture` (crashes Debug builds)**
    A future stored in `IMAPFetchAttempts` is deallocated without ever being completed, tripping NIO's debug-only `EventLoopFuture.deinit` assertion and killing the app.
    > **Discovered 2026-08-24** during the item 61 live loop observation: a locally-run **Debug** build crashed mid-session (`EXC_BREAKPOINT`/`SIGTRAP`) ~16 min into watching, after successfully drafting + saving a test reply. Backtrace: `_assertionFailure` → `EventLoopFuture.deinit` (via `debugOnly`) → `outlined destroy of [ObjectIdentifier: EventLoopFuture<[MailMessage]>]` → `IMAPFetchAttempts.deinit` → `IMAPMailProvider.fetchRecentMessages(_:mailbox:limit:)` → `AppState.fetchWatcherMessages` → `RetryRunner.run`. The assertion is `debugOnly`, so it is **compiled out of Release builds** — shipped signed/notarized binaries do not trap — but it indicates a real resource-management bug: an `EventLoopFuture<[MailMessage]>` cached in `IMAPFetchAttempts` is dropped without being fulfilled/consumed, and it destabilizes Debug-build dogfooding.
    *As the maintainer, I want the watcher's IMAP fetch to never leak an uncompleted future, so that dogfooding a Debug build is stable and the fetch path is correct (no orphaned promises/connections) in Release too.*
    - Audit `IMAPMailProvider.fetchRecentMessages` and `IMAPFetchAttempts`: every `EventLoopFuture`/`EventLoopPromise` stored (keyed by `ObjectIdentifier`) is either awaited/consumed or explicitly failed/cancelled before the holder deinits — no future is abandoned, including on the error, timeout, empty-result, and connection-teardown paths.
    - The specific `[ObjectIdentifier: EventLoopFuture<[MailMessage]>]` cache is drained or its pending entries completed on teardown so `deinit` never destroys an unfulfilled future.
    - A Debug build survives extended watching (repeated `fetchRecentMessages` cycles, including induced fetch errors/timeouts) with no `EventLoopFuture.deinit` assertion.
    - Regression coverage: a test that exercises the fetch/teardown path (including a failing/timed-out fetch) and asserts no promise is leaked (NIO's leak assertions stay silent under Debug).
