    ...
    - **Phasing (added 2026-08-20):**
      ...
      - **56b — Metering + limits:** per-account token metering, caps with the fair-use policy, rate limiting, abuse controls, margin dashboard for the maintainer; the app shows remaining allowance and the "switch to your own key for unlimited" valve.
        ...
        - **Build slices (scoped 2026-08-29):**
          ...
          3. ✅ **App — usage surfacing + alerts** *(done 2026-08-29, branch `usage-metering`)*: `ManagedInferenceClient`/`ManagedAccountService` decode `quota` from `/v1/me` and each `/v1/draft`; map `429 quota_exceeded` / `429 rate_limited` / `413` to new `LLMError` cases with friendly copy (rate-limit → retry-after message; quota in hard mode → "buy more" CTA placeholder pointing at 56c + the item 59 "use your own key for unlimited" valve). The Settings → AI Provider managed pane shows **"N of M drafts used this week · resets <day>"** with a progress bar and the own-key link. **Alerts at 50/75/100%** fire once per weekly window per threshold via `NotificationService` (state persisted per `resetsAt` so a relaunch doesn't re-fire); the 100% notice says drafting continues (soft mode) and points to buying extra usage when 56c lands. Prowl hunt mode: stubbed quota, zero network.
             - **Delivered:** `ManagedQuota` value type (lenient Codable, optional on both endpoints); quota threaded to `AppState.managedQuota` via a `ManagedQuotaRelay` and refreshed at launch/sign-in/settings-open (`GET /v1/me`) plus every `/v1/draft`; new `LLMError` cases + friendly copy (`AppState.llmMessage`), `Retry-After` header fallback, `ResilienceClassifier` mapping (rate-limit transient, quota/too-large permanent); `ManagedUsageView` pane (progress + reset time + extra-usage line + buy-more placeholder + own-key valve, AX ids); once-per-window 50/75/100% alerts (`UsageAlertEvaluator` + `UserDefaultsUsageAlertStore` + `NotificationService` `USAGE_ALERT` category opening Settings → AI); hunt-mode stubbed quota. Tests: `ManagedQuotaTests`, `UsageAlertTests`, `AppStateUsageQuotaTests`, extended `ManagedInferenceClientTests`, gated live `/v1/me` quota check.
