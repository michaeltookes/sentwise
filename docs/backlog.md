...
57. **Landing page / marketing site — remaining launch cut (checkout CTA, download, waitlist, feedback form)** — *site is LIVE at sentwise.ai; the concrete work is tracked in the `sentwise-landing-page` repo's own backlog, whose items decompose this one*
    The public site where people find the product, understand it in 30 seconds, and pay. **Status 2026-09-12 — shipped and live** (Next.js 16 / Tailwind v4 on Vercel, Cloudflare DNS; "Closer" emerald+gold brand): home page with positioning + hero, informational `/pricing` (Starter $9 / Pro $19 "Most popular" / Unlimited $39 — owner-confirmed 2026-09-04; Team/Enterprise deferred), `/faq`, the four legal pages (`/terms`, `/privacy`, `/security`, `/support`), SEO fundamentals + sitemap/robots, and branded OG/social cards. The hosted policies backed the now-complete Paddle vendor verification. Copy discipline holds: the training-data contrast is stated accurately, model-provider zero-retention is NOT asserted pending Anthropic ZDR terms in writing, and "we never even see your calls" is confined to the BYO/local tier.
    *As Marcus, I want to find the product, get what it does in 30 seconds, and start a trial without friction, so that trying it is easier than ignoring it.*
    Remaining for the launch cut (site-repo item numbers in parentheses):
    - **Checkout wiring (site item 5) — decided 2026-09-12: all tier CTAs funnel to the download** (subscribe happens in-app via the working 56c Paddle checkout); site-hosted checkout is explicitly out of the launch cut. This collapses the site work into item 6's download CTA — no Paddle price-id wiring on the site.
    - **Download & trial CTA (site item 6):** the site-wide `TrialButton` is a deliberate non-interactive "Trial coming soon" — replace with the signed/notarized DMG download (and/or Homebrew cask), consistent with the app's sign-in-and-go onboarding (item 59); macOS requirement stated with the Windows-waitlist secondary path.
    - **"Windows — join the waitlist"** email capture (site item 7) — the demand probe for a future cross-platform port.
    - **Feedback form (site item 8; moved here from item 36, 2026-08-26):** non-bug feedback/feature requests with a form backend; complements — does not replace — the in-app "Report a Problem" path, which owns bug reports because only the app produces the redacted diagnostic bundle; the app may deep-link with app/macOS version prefilled.
    - Demo video home (site item 9); social proof only when real evidence exists (site item 11).
    - This item closes when the launch cut — checkout + download at minimum — is live on the site.

72. **Legal, policy, and support foundations for a paid launch — residuals** — *the 2026-08-30 hold was overtaken by events: the bundle shipped in the `sentwise-landing-page` repo (its item 4, 2026-09-03) and backed the completed Paddle vendor verification*
    **Status 2026-09-12 — delivered and live on sentwise.ai** (versioned in the site repo): **Terms of Service** (fair-use caps, trial terms, Paddle-as-Merchant-of-Record billing, refund section anchored as `#refunds` and aligned with Paddle's 14-day handling), **Privacy Policy** (accurate to the local-first architecture; managed-plan "never used to train models" stated; model-provider zero-retention deliberately NOT asserted), **Security** page, and **Support** page (two-business-day response expectation). `support@sentwise.ai` is a real, monitored mailbox — a Google Group on the Workspace secondary domain with SPF/DKIM/DMARC configured (see the site repo's `docs/email-setup.md`); security/privacy queries route there by subject line (owner decision — no separate `security@` alias).
    *As a prospective customer, I want to read exactly what Sentwise does with my mail, my calls, and my payment, and know where to get help, so that paying for it feels safe.*
    Remaining:
    - **Anthropic ZDR terms — DEFERRED until post-launch traction (owner decision 2026-09-12, supersedes "pending"):** as a solo pre-revenue founder the ZDR request would likely go unanswered/declined, and nothing depends on it — the softened copy is the **standing** copy, accurate as-is (no-training already backed by Anthropic's standard Commercial Terms; privacy maximalists are served by the BYO/local tier). Revisit with Anthropic Sales when there's revenue/volume to point at; until then this is a closed decision, not an open loop. *(Original context below.)* Previously: still the gate for strengthening the Privacy Policy and marketing copy to full zero-retention wording (item 56 decision); the honest softened copy stays until then. **What this concretely is (clarified 2026-09-12):** the "never used to train models" half of the claim is already backed in writing by Anthropic's standard Commercial Terms (API inputs/outputs are not used for training); the "nothing stored" half is not — the standard API retains data (~30 days) for trust & safety. **Zero Data Retention is a per-organization agreement granted by Anthropic** on review. Action: contact Anthropic Sales, name the Anthropic organization whose API key backs `sentwise-inference`, request ZDR for it, and get the agreement/addendum in writing; scrutinize carve-outs (trust-&-safety flags, models requiring 30-day retention, features needing storage). A solo/small org may not qualify — if declined, the softened copy simply becomes permanent and the differentiator rests on the no-training guarantee + the BYO/local tier.
    - ✅ **Owner review done (2026-09-12):** the owner read every hosted policy page. No professional legal review — solo operation, accepted risk (note honestly: Paddle verification is a payments/compliance screen, not a legal review); revisit counsel post-revenue if warranted.
    - **In-app Legal & Support links** (Settings/About → the hosted `/terms`, `/privacy`, `/security`, `/support` pages).
    - Fair-use cap final numbers ride the 56b measure-first tuning (ToS wording already accommodates them).
