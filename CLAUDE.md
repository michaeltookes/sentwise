...
- **Monetization (updated 2026-08-12, second revision — supersedes "no subscription" and BYO-key-as-default; BYO-key/local parked 2026-09-16, item 100):** subscription with **managed inference bundled** (user never touches an API key; stateless zero-retention proxy; license = account); open core / paid binary — source stays public, signed auto-updating binaries are licensed. Trial → Individual → Team. The BYO-key/local path is no longer part of the UI or pricing story — it is parked in the open-core source only (unsupported). See backlog items 56–59 and 100. The landing page/marketing site lives in **its own repo** and requires a discussion before any building starts.
