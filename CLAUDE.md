...
**Direction update (2026-08-12):** the **flagship workflow is the post-call follow-up** — when a call ends, ingest the transcript and draft the next-steps email in the user's voice (backlog items 51–57). Inbox reply drafting remains, as one workflow among several. Primary commercial ICP: **Account Executives / high-velocity salespeople** (the "Marcus" persona in `docs/backlog.md`). **No bot, no storage, no training:** calls are never joined by a bot; audio and (future) capture/transcription stay on-device; call content is never stored server-side or used as training data. Drafting runs through a stateless managed proxy where Sentwise does not log or retain request/response bodies (2026-08-12 managed-inference decision — backlog items 56/59; BYO-key/local drafting parked 2026-09-16, item 100, so the managed proxy is the only shipped path). (Bot-free capture alone is no longer unique — Fathom and Granola both offer it but store calls in their cloud and train on de-identified customer data; the durable differentiators are no Sentwise storage/training and the send-ready email in the user's voice from their own mailbox.)
...
- **LLM access:** pluggable provider architecture — **managed inference is the only shipped path** (bundled into the subscription, no keys; backlog item 56). *(Superseded 2026-09-16 — item 100: the BYO-any-provider and local-model (Ollama) paths, once the "power/privacy" option under item 59, were **parked** — removed from the UI, onboarding, and pricing. The provider seam, the non-managed `LLMProviderKind` cases, `AnthropicClient`, and `OpenAICompatibleClient` remain in the open-core source, compiling and unit-tested but unreachable from the UI, for possible future revival; any persisted non-managed selection falls back to managed at launch.)* **56a shipped (2026-08-20):** the `.managed` provider (`LLMProviderKind.managed`, default for new installs; `ManagedInferenceClient`) drafts through the stateless **`sentwise-service`** Cloudflare Worker (`sentwise-inference`, deployed at `https://sentwise-inference.sentwise-service.workers.dev`) under a Clerk account session, with request/response bodies held in memory only and not logged or retained by Sentwise. Auth is **Clerk** (email-code sign-in via the Frontend API, implemented natively in the app — not the clerk-ios SDK; see `docs/managed-inference.md`); the model provider is the **Anthropic Messages API** (`claude-sonnet-4-6` default); the 14-day trial, monthly metering/caps (56b), and Paddle checkout/licensing plus billing portal flows (56c) are enforced server-side.
- **Ethos:** local-first, private. Mail data, voice profile, and (future) call audio stay on the machine; drafting inference leaves only through Sentwise's stateless managed proxy, which does not log or retain request/response bodies — the sole shipped path since 2026-09-16 (item 100 parked the BYO-key/local option). Secrets live in the macOS Keychain.
- **Monetization (updated 2026-09-23 for the 1.0 launch; supersedes "no subscription," BYO-key-as-default, and the earlier Individual/Team framing; BYO-key/local parked 2026-09-16, item 100):** subscription with **managed inference bundled** (user never touches an API key; Sentwise no-body-logging managed proxy; license = account); open core / paid binary — source stays public, signed auto-updating binaries are licensed. Trial → Starter → Pro → Unlimited; `team` remains a reserved plan value and is not sold. The BYO-key/local path is no longer part of the UI or pricing story — it is parked in the open-core source only (unsupported). See backlog items 56–59, 98, and 100. The landing page/marketing site lives in **its own repo** and requires a discussion before any building starts.
