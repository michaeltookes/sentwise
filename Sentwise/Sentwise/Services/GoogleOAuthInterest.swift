...
/// without any network.
struct GoogleOAuthInterestRegistration: Equatable, Sendable {
    let accountKey: String?
}

struct GoogleOAuthInterestRegistrationFailure: Error {
    let accountKey: String?
    let underlying: Error
}

extension GoogleOAuthInterestRegistrationFailure: LocalizedError {
...
struct GoogleOAuthInterestClient: GoogleOAuthInterestRegistering {
    ...
    func registerInterest(topic: String) async throws -> GoogleOAuthInterestRegistration {
        ...
        do {
            ...
            throw GoogleOAuthInterestRegistrationFailure(
                accountKey: session.accountKey,
                underlying: LLMError.invalidResponse("Couldn't encode the interest request. (\(error))")
            )
        }
        ...
        do {
            ...
            throw GoogleOAuthInterestRegistrationFailure(
                accountKey: session.accountKey,
                underlying: LLMError.transport(String(describing: error))
            )
        }
        ...
        guard response.isSuccess else {
            throw GoogleOAuthInterestRegistrationFailure(
                accountKey: session.accountKey,
                underlying: ManagedInferenceClient.mapError(
        ...
        return GoogleOAuthInterestRegistration(accountKey: session.accountKey)
    }
