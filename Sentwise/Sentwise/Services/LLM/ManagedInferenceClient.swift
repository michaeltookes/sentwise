...
struct ManagedInferenceClient: LLMClient {
    ...
    /// `POST /v1/paddle/checkout` (backlog item 56c). Subscriptions require
    /// quantity 1 (the Worker enforces this); the returned `transactionId` is
    /// what the app hands to `Paddle.Checkout.open({ transactionId })`. The
    /// transaction already carries the signed `custom_data` + customer binding
    /// server-side, so the webhook attributes the completed purchase reliably.
    /// Throws `LLMError.managedNotSignedIn` on `401` and
    /// `LLMError.managedCheckoutFailed` (with the Worker's plain message) on
    /// other non-2xx responses (`400 invalid_request`, `404 account_not_found`,
    /// `409` eligibility errors, `503 checkout_unavailable`, `502`).
    ...
    /// Maps a `POST /v1/paddle/checkout` failure into a clear `LLMError` (backlog
    /// item 56c). `401` is "not signed in"; every other non-2xx surfaces the
    /// Worker's plain, user-facing message (which is already scrubbed of upstream
    /// Paddle/Clerk detail), falling back to a generic checkout message.
